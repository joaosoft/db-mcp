...
func (s *DatabaseMCP) toolConfigureDataSource() (mcp.Tool, server.ToolHandlerFunc) {
	return mcp.Tool{
...
func (s *DatabaseMCP) handleConfigureDataSource(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if !ok || driver == "" {
		return mcp.NewToolResultError("Driver is required"), nil
	}

	connString, ok := args["connection_string"].(string)
	if !ok || connString == "" {
		return mcp.NewToolResultError("Connection string is required"), nil
	}
	...
	switch driver {
	case "sqlite":
		normalizedDriver = "sqlite3"
	case "oracle":
		normalizedDriver = "godror"
	}

	if !validDrivers[driver] && !validDrivers[normalizedDriver] {
		return mcp.NewToolResultError(fmt.Sprintf("Invalid driver '%s'. Supported drivers: sqlserver, postgres, mysql, sqlite, oracle", driver)), nil
	}

	// Try to connect
	newDB, err := sql.Open(normalizedDriver, connString)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("Failed to open connection: %v", err)), nil
	}
	...
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("Error serializing response: %v", err)), nil
	}
...
func (s *DatabaseMCP) toolGetCurrentDataSource() (mcp.Tool, server.ToolHandlerFunc) {
	return mcp.Tool{
...
func (s *DatabaseMCP) handleGetCurrentDataSource(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if !exists {
		return mcp.NewToolResultError("Connection info not found"), nil
	}
...
func (s *DatabaseMCP) toolTestConnection() (mcp.Tool, server.ToolHandlerFunc) {
	return mcp.Tool{
...
func (s *DatabaseMCP) handleTestConnection(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	args, ok := request.Params.Arguments.(map[string]interface{})
	if !ok {
		return mcp.NewToolResultError("Invalid arguments"), nil
	}
...
// Tool: Disconnect
func (s *DatabaseMCP) toolDisconnect() (mcp.Tool, server.ToolHandlerFunc) {
	return mcp.Tool{
...
func (s *DatabaseMCP) handleDisconnect(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	if s.db == nil {
		return mcp.NewToolResultError("No active database connection"), nil
	}
...
func (s *DatabaseMCP) toolListDrivers() (mcp.Tool, server.ToolHandlerFunc) {
	return mcp.Tool{
...
func (s *DatabaseMCP) handleListDrivers(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
